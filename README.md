...
* **[Housing & Labor Unified Registry](https://national-housing-labor-registry.a-8c6.workers.dev)** — the national site: generic Socrata/ArcGIS/Carto platform adapters scrape rental-registry and violation feeds from cities across the country (NYC, Seattle, Philadelphia, Detroit, Austin, San Francisco, Denver, Nashville, Cincinnati, Buffalo, + Twin Cities; Milwaukee, Pittsburgh, and NJ via ingest scripts) into the same D1/R2 store the three Twin Cities workers below read from. Adding a city = one adapter file. Unified city browser, `/api/*` endpoints (landlords, entities, wage-theft), and the dual-offender crossover join (`/crossover` UI, `/api/crossover` live D1 join + curated dossier).
* **[Slumlord & Wage Theft Crossover Matrix](https://twin-cities-slumlord-labor-matrix.a-8c6.workers.dev)** — legacy Twin Cities view: a curated dossier of corporate syndicates cited for both housing code violations and wage theft. Its data and UI are embedded in the unified registry above (`/crossover`); this standalone deployment keeps the original `/matrix` API and in-browser AI assistant.
* **[Bruenig Chart Updates](https://bruenig-chart-updates.a-8c6.workers.dev)** ([source](bruenig-chart-updates/)) — recomputes Matt Bruenig / People's Policy Project charts on current data: market-vs-disposable poverty (CPS ASEC), wealth concentration (SCF), and US-vs-OECD poverty comparisons. A monthly cron watches each dataset's index page and flags charts stale the day a new vintage drops.
