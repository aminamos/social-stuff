...
export default {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/sync") {
      ...
      ctx.waitUntil(syncRegistry(env));
      return new Response(JSON.stringify({ status: "Sync initiated in background" }), {
    ...
    return new Response("Minneapolis Rental Housing Registry Worker. Endpoints: /sync, /stats", { status: 200 });
  },
};
...
async function syncRegistry(env: Env): Promise<void> {
  ...
  while (true) {
    ...
    for (const f of features) {
      ...
      // Execute D1 in batches of 100 statements
  ...
  // Upload compressed daily snapshot to Cloudflare R2
