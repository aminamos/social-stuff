...
Deno.test("export.csv keeps comma fields inside their columns", async () => {
  const env = envWithRow({
    ...
    source_docket_url:
      "https://twin-cities-wage-theft-worker.a-8c6.workers.dev/docs/WHD-DOL-1364653.pdf",
  });
  const res = await (worker as any).fetch(
    new Request("https://twin-cities-wage-theft-worker.a-8c6.workers.dev/export.csv"),
    env,
  );
