...
class DossierGeneratorAgent:
    ...
    def generate_dossier(self, investigation_result: Dict[str, Any]) -> str:
        ...
        wage_theft = investigation_result.get("wage_theft_records", [])
        if wage_theft:
            lines.extend([
                "---",
                "",
                "## 5. Labor Exploitation & Wage Theft Crossover (Joint Labor-Tenant Leverage)",
                "",
                "The corporate landlord syndicate and its operational management affiliates have documented labor enforcement actions, citations, or settlements on record:",
                "",
                "| Employer / Target | Enforcement Agency | Violation Type | Back Wages Recovered | Civil Penalties | Workers Affected | Case Status |",
                "| :--- | :--- | :--- | :--- | :--- | :--- | :--- |",
            ])
            for wt in wage_theft:
                name = wt.get("respondent_legal_name") or wt.get("trade_name") or "Unknown"
                agency = wt.get("source_agency", "US_DOL_WHD")
                vtype = wt.get("violation_type", "FLSA_OVERTIME")
                bw = wt.get("back_wages_recovered", 0.0)
                pen = wt.get("civil_penalties_assessed", 0.0)
                workers = wt.get("workers_affected", 0)
                status = wt.get("status", "VIOLATION_CONFIRMED")
                lines.append(f"| **{name}** | `{agency}` | `{vtype}` | ${bw:,.2f} | ${pen:,.2f} | {workers} workers | `{status}` |")
            lines.extend([
                "",
                "### Strategic Worker-Tenant Alliance:",
                "Exploitative landlords rarely exploit only one constituency. Chronic building code neglect is almost universally paired with wage theft against on-site caretakers, turnover cleaning staff, and maintenance technicians. Connecting with the affected workers and labor unions (e.g. SEIU Local 26, CTUL) transforms an atomized tenant grievance into an unstoppable citywide coalition.",
                "",
            ])
        ...
        lines.extend([
            ...
            "3. **Joint Worker-Tenant Pickets**: Coordinate with building maintenance staff, janitors, and worker centers. Concurrent unfair labor practice (ULP) strikes and tenant rent withholding create multi-front operational paralysis.",
            "4. **Joint City Council & Inspection Demand**: File a consolidated petition with the Department of Regulatory Services demanding portfolio-wide inspection audits, rather than treating each building as an isolated one-off landlord.",
            "5. **Lender Notification Campaign**: Send formal certified notices of hazardous housing code violations to the master mortgage servicers and trustees.",
            "",
